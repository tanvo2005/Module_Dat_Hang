            ...
            <div class="myorder_ContentNavabar">
                <ul class="myorder_ContentNavabar_list">
                    ...
                    <a href="{{ route('index') }}" style="text-decoration: none;color: black">
                        <li class="myorder_ContentNavabar_item">Home</li>
                    </a>
                    ...
                    <div class="myorder_Content_product">
                                ...
                                <div class="myorder_Content_productContent">
                                    <div class="myorder_Content_productContentDecripsion">
                                        ...
                                        <img src="{{ asset($imgPath) }}" alt=""
                                            class="myorder_Content_productContent_img">
                                        <div class="myorder_Content_productContent_decription">
                                            <span
                                                class="myorder_Content_productContent_title">{{ $detail->variant->product->productname }}</span>
                                            <span class="myorder_Content_productContent_classi">Phân Loại :
                                                {{ $detail->variant->color }}, {{ $detail->size }}</span>
                                            <span
                                                class="myorder_Content_productContent_quantity">x{{ $detail->quantity }}</span>
                                        </div>
                                    </div>

                                    <div class="myorder_Content_productContentPrice">
                                        <span
                                            class="myorder_Content_productContentPriceNew">{{ number_format($detail->unitprice * $detail->quantity) }}đ</span>
                                    </div>
                                </div>
                            @endif
                        @endforeach
                        ...
                        <div class="myorder_Content_productPayBill">
                            <div class="myorder_Content_productPayBill_sum">
                                ...
                                <span
                                    class="myorder_Content_productPayBill_price">{{ number_format($order->totalamount) }}đ</span>
                            </div>
                            ...
                            <div class="myorder_Content_productPayBill_btn">
                                @if ($order->status == 0)
                                    <form action="{{ route('order.cancel', $order->order_id) }}" method="POST"
                                        ...
                                        @csrf
                                        <button type="submit" class="btn btn--normal myorder_Content_productPayBill_btnNew"
                                ...
                                @endif
                                <a href="{{ route('myOrderDetail', $order->order_id) }}"><button
                                        class="btn btn--primary myorder_Content_productPayBill_btnNew"> Xem chi
                                        tiết</button></a>
                                @if ($order->status == 3)
                                    <button class="btn btn--primary myorder_Content_productPayBill_btnNew"> Mua lại</button>
                                    <button class="btn btn--primary myorder_Content_productPayBill_btnNew"> Liên hệ người
                                        bán</button>
                                @endif
                                @if ($order->status == 4)
                                    <button class="btn btn--primary myorder_Content_productPayBill_btnNew"> Mua lại</button>
