...
@section('content')
    ...
    @php
        // Bảng quy đổi: Giá trị trong DB => Chữ hiển thị
        $sizeMap = [
            1 => 'XS',
            2 => 'S',
            3 => 'M',
    ...
    @if ($cart && $cart->details->isNotEmpty())
                        ...
                        <div class="cartDetail_content">
                            <div class="cartDetail_content5 cartDetail_PhanLoai">
                                ...
                                <span class="cartDetail_PhanLoaiMota">{{ $detail->variant->color }},
                                    {{ $sizeMap[$detail->variant->size] ?? $detail->variant->size }}</span>
                            </div>

                            <div class="cartDetail_content5 cartDetail_price" data-price="{{ $detail->unitprice }}">
                                {{ number_format($detail->unitprice) }}đ</div>

                            <div class="cartDetail_content5 cartDetail_quantity">
                                <input class="productDeatil_QuantityInt quantity-input" type="number"
                                    value="{{ $detail->quantity }}" min="1" max="{{ $detail->variant->quantity }}"
                                    data-id="{{ $detail->cartdetail_id }}">
                            </div>

                            <div class="cartDetail_content5 cartDetail_sumprice"
                                id="item-subtotal-{{ $detail->cartdetail_id }}">{{ number_format($itemTotal) }}đ</div>

                            <div class="cartDetail_content5 cartDetaiProductDelete">
                                <button style="background-color: #d5d5d5;" type="button"
                                    class="cartDetaiProductDelete_btn delete-item-btn"
                                    data-id="{{ $detail->cartdetail_id }}">Xoá</button>
                            </div>
                        </div>
                    </div>
                @endif
        ...
        <div class="cartDetailend">
            <div class="cartDetailend_MuaHang">
                <span class="cartDetailend_MuaHang_quantity" id="grand-total-quantity">Tổng Cộng
                    ({{ $cart->details->sum('quantity') }} Sản Phẩm):</span>
    ...
    @else
        ...
        <div class="cartDetail" style="text-align: center;padding: 50px;height: 100%;display: block;">
            ...
            <a href="{{ route('index') }}" class="home-filter-btn btn btn--primary" style="margin-top: 20px;">Quay lại
                Trang chủ</a>
        </div>
    @endif
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function updateCartQuantity(cartDetailId, newQuantity) {
                try {
                    ...
                    if (response.ok && result.success) {
                        ...
                        document.getElementById('grand-total-quantity').textContent =
                            `Tổng Cộng (${result.totalQuantity} Sản Phẩm):`;
                        document.getElementById('grand-total-price').textContent = formatPrice(result
                            .totalPrice) + 'đ';
            ...
            async function deleteCartItem(cartDetailId) {
                ...
                try {
                    ...
                    if (response.ok && result.success) {
                        ...
                        const totalQuantity = Array.from(document.querySelectorAll('.quantity-input')).reduce((
            ...
            document.querySelectorAll('.quantity-input').forEach(input => {
                input.addEventListener('input', function() {
                    ...
                    const price = parseFloat(this.closest('.cartDetail_content').querySelector(
                        '.cartDetail_price').dataset.price);
